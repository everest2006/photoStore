...
export const renameFolder = (req, res) => {
    parseFromData(req).then(function (dataFromRequest) {
        renameInYandex(dataFromRequest.oldFolderName, dataFromRequest.newFolderName,dataFromRequest.pathFolder)
            .then(function (status) {
                log.info(`folder ${dataFromRequest.newFolderName} rename to Yandex disk  ${status}`);
                return new Promise(function (resolve, reject) {
